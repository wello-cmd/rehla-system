...
router.get('/pl', authenticate, async (req, res) => {
  try {
    ...
    // COGS from order_items × cost_per_unit (FR-FN-06)
    let itemsQuery = supabase.from('order_items').select('quantity, cost_per_unit, order_id');
    const { data: items } = await itemsQuery;
    const cogs = (items || []).reduce((s, i) => s + (Number(i.quantity) * Number(i.cost_per_unit || 0)), 0);

    // Gross Profit (FR-FN-05)
